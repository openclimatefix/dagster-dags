...
ecmwf_uk_partitions = dg.MultiPartitionsDefinition({
...
@dg.asset(
    name="source_archive",
    key_prefix=["nwp","ecmwf","uk"],
    partitions_def=ecmwf_uk_partitions,
    check_specs=[
        dg.AssetCheckSpec(name="nonzero_num_files", asset=['nwp', 'ecmwf', 'uk', 'source_archive'])
    ],
    compute_kind="network_request",
    op_tags={"MAX_RUNTIME_SECONDS_TAG": 1000}
)
def ecmwf_uk_source_archive(
        context: dg.AssetExecutionContext
) -> dg.Output[list[FileInfoModel]]:
    """Asset detailing all wanted remote files for ECMWF UK."""
...
@dg.asset(
    ...
    partitions_def=ecmwf_uk_partitions,
    check_specs=[
        dg.AssetCheckSpec(name="num_local_is_num_remote", asset=['nwp', 'ecmwf', 'uk', 'raw_archive']),
        dg.AssetCheckSpec(name="nonzero_local_size", asset=['nwp' , 'ecmwf', 'uk', 'raw_archive']),
    ],
    metadata={
        "archive_folder": dg.MetadataValue.text(f"{RAW_FOLDER}/nwp/ecmwf/uk"),
        "area": dg.MetadataValue.text("uk"),
    },
    compute_kind="download",
    op_tags={"MAX_RUNTIME_SECONDS_TAG": 1000}
)
def ecmwf_uk_raw_archive(
    context: dg.AssetExecutionContext,
    source_archive: list[FileInfoModel]
) -> dg.Output[list[pathlib.Path]]:
    """Locally stored archive of raw data from ECMWF for the UK."""
