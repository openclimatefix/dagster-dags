...
and 30-minutely. This asset pulls the 30-minutely data.
...
@dg.asset(
    name="passiv_30min_yearly",
    description="Yearly summary of 30-minutely PV data from Passiv systems.",
    metadata={
        "source": dg.MetadataValue.text("sheffield-solar"),
        "area": dg.MetadataValue.text("uk"),
        "expected_runtime": dg.MetadataValue.text("1 hour"),
    ...
    ins={"passiv_30min_monthly": dg.AssetIn(partition_mapping=dg.TimeWindowPartitionMapping())},
    io_manager_key="parquet_io_manager",
)
def pv_passiv_30min_yearly_asset(
    context: dg.AssetExecutionContext,
    passiv_30min_monthly: dict[str, pd.DataFrame],
) -> pd.DataFrame:
    """Dagster asset summarising yearly 30-minutely PV data from Passiv systems."""
    context.log.info("Summarizing monthly data for {context.partition_time_window.start:%Y}")
    return pd.concat(passiv_30min_monthly.values())
